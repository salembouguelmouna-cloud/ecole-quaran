...
if (process.env.DATABASE_URL) {
  ...
  sess.store = new PgSession({ pool: db.pool, createTableIfMissing: true });
}
...
app.use((err, req, res, next) => {
  ...
  res.status(500).send('<!DOCTYPE html><html dir=rtl lang=ar><head><meta charset=UTF-8><title>خطأ</title><script src=https://cdn.tailwindcss.com></script></head><body class="flex items-center justify-center min-h-screen bg-gray-100"><div class="bg-white p-8 rounded-2xl shadow-lg text-center max-w-md"><div class="text-6xl mb-4">⚠️</div><h1 class="text-2xl font-bold text-gray-800 mb-2">حدث خطأ غير متوقع</h1><p class="text-gray-600 mb-6">الرجاء المحاولة مرة أخرى لاحقًا</p><a href="/login" class="bg-emerald-600 text-white px-6 py-2.5 rounded-xl hover:bg-emerald-700">العودة للصفحة الرئيسية</a></div></body></html>');
});
